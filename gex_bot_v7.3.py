...
GEX_SIGN_MODEL = os.getenv("GEX_SIGN_MODEL", "SIMPLE").upper()
...
HEARTBEAT_HOUR  = int(os.getenv("HEARTBEAT_HOUR","0"))
...
def to_text(payload, prev=None, compact=False):
    ...
    if compact:
        ...
        lines.append(f"Bias: {p['bias_line']} | NetΓ: {human_gex(p['net_gex_smoothed'])} ({p['net_gex_sign']}) | Flip ~{fmt_compact_price(p['flip_zone']) if p['flip_zone'] else '—'}")
        edges = p.get("edges", [])
        if edges:
...
def should_post(payload, prev):
    if prev is None: return True
    if POST_ON_SIGN_CHANGE and payload["net_gex_sign"] != prev.get("net_gex_sign"):
        return True
    if POST_ON_EDGE_CHANGE and nearest_edge_val(payload) != nearest_edge_val(prev):
        return True
    if POST_ON_MOVE_USD>0 and abs(payload["spot"] - prev.get("spot", payload["spot"])) >= POST_ON_MOVE_USD:
        return True
    return True
...
def run_once(silent=False, outfile=None, send_telegram=False, statefile=None, compact=False, pretty=False, ultra=False):
    spot = get_spot()
    chain = fetch_chain()
    ...
    if pretty:
        text = to_html(payload, prev=prev); parse = "HTML"
    elif ultra:
        text = to_ultra(payload, prev=prev); parse = None
    else:
        text = to_text(payload, prev=prev, compact=compact or (not pretty and not ultra)); parse = None

    if not silent:
        print(text, flush=True)

    if send_telegram and should_post(payload, prev):
        sent, info = telegram_send(text, parse_mode=parse)
        if (not sent) and pretty and "can't parse entities" in info:
            text_fallback = to_text(payload, prev=prev, compact=True)
            telegram_send(text_fallback, parse_mode=None)
            if not silent:
                print("[telegram] HTML parse failed, sent compact fallback.", flush=True)
        elif not sent and not silent:
...
if __name__ == "__main__":
    ...
    p.add_argument("--ultra", action="store_true", help="One-line, ultra-compact summary")
    args = p.parse_args()
