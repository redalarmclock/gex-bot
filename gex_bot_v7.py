...
MIN_GEX_CHANGE_M = float(os.getenv("MIN_GEX_CHANGE_M", "0.10"))         # show gex delta >= 0.10M
...
def build_bias_line(spot, net_smoothed, flip_zone, pos_min):
    ...
    if flip_zone is None and pos_min is None:
        return f"Bias: {sign} gamma. Monitor shifts."
...
def to_text(payload, prev=None, compact=False):
    ...
    if compact:
        ...
        # nearest/strongest levels (abbrev)
        def lvl(x): 
            if not x: return "—"
            lr = "+" if x["gex"]>=0 else "−"
    ...
    def line_for(x, title):
        if not x: return f"{title}: —"
        dist = x["strike"] - spot; pct = (dist / spot) * 100.0
        return (f"{title}: {fmt_num(x['strike'])} ({'Support (+)' if x['gex']>=0 else 'Resistance (−)'}, "
                f"GEX {human_gex(x['gex'])}, Δ={fmt_num(dist)} / {pct:.2f}%)")

    lines.append(line_for(p["nearest_below"], "Nearest Below"))
    lines.append(line_for(p["nearest_above"], "Nearest Above"))
    lines.append(line_for(p["strongest_below"], "Strongest Below"))
    lines.append(line_for(p["strongest_above"], "Strongest Above"))

    if p.get("flip_zone"):
        lines.append(f"GEX Flip Zone (−→+): ~{fmt_num(p['flip_zone'], PRECISION)}")
    lines.append(f"Net Gamma (smoothed): {human_gex(p['net_gex_smoothed'])}  |  Sign: {p['net_gex_sign']}")
    if p.get("bias_line"):
        lines.append("🧭 " + p["bias_line"])

    # one nearest strong edge
    edges = p.get("edges", [])
    if edges:
        e = sorted(edges, key=lambda x: abs(x["edge"]-spot))[0]
        dist = e["edge"] - spot
        lines.append("")
        lines.append("Nearest cluster edge:")
        lines.append(f"• ~{fmt_num(e['edge'],0)}  | Δ={fmt_num(dist,0)} ({(100*dist/spot):.2f}%)  | strength {fmt_num(e['strength'],0,False)}")

    lines.append("")
    lines.append(f"Sticky levels (|GEX|, Top-{TOP_N}):")
    for x in p["stickies_topN"]:
        lines.append(f"• {fmt_num(x['strike'])}  {'Support (+)' if x['gex']>=0 else 'Resistance (−)'}  |  GEX {human_gex(x['gex'])}")

    lines.append("")
    lines.append("Trade map:")
    lines.append(p["trade_map"])
...
def run_once(silent=False, outfile=None, send_telegram=False, statefile=None, compact=False):
    spot = get_spot()
    chain = fetch_chain()
    ...
    text = to_text(payload, prev=prev, compact=compact)
    if not silent:
...
if __name__ == "__main__":
    ...
    if args.loop:
        while True:
            try:
                run_once(silent=args.silent, outfile=args.outfile or "", send_telegram=args.telegram, statefile=args.statefile, compact=args.compact)
            except Exception as e:
                if not args.silent:
